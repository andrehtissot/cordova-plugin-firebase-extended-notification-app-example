(function() {
  ...
  function getDisabledOptionsNamesFromForm() {
    ...
    for (var extractionAttribute in inputsNamesByExtractionAttribute)
      for (
        var i =
          inputsNamesByExtractionAttribute[extractionAttribute].length - 1;
        i >= 0;
        i--
      )
        if (
          document.getElementsByName(
            inputsNamesByExtractionAttribute[extractionAttribute][i]
          )[0].disabled
        )
  ...
  function getOptionsFromForm(formatOptions) {
    var formatOptions = formatOptions || {},
      options = {},
      filterDisabled =
        "filter" in formatOptions &&
        formatOptions.filter.indexOf("disabled") !== -1,
      filterHidden =
        "filter" in formatOptions &&
        formatOptions.filter.indexOf("hidden") !== -1;
    for (var extractionAttribute in inputsNamesByExtractionAttribute) {
      for (
        var i =
          inputsNamesByExtractionAttribute[extractionAttribute].length - 1;
        ...
        if (
          (filterDisabled && input.disabled) ||
          (filterHidden && input.offsetHeight === 0)
        ) {
          continue;
        }
        options[inputsNamesByExtractionAttribute[extractionAttribute][i]] =
          input[extractionAttribute];
      }
    }
    if (
      "textLines" in options &&
      "textLinesAs" in formatOptions &&
      formatOptions.textLinesAs === "arrayOfLines"
    )
      options.textLines = options.textLines.split("\n");
    if (
      !filterDisabled ||
      !document.getElementById("notificationOptionsVibrationOption").disabled
    ) {
      if (
        document.getElementById("notificationOptionsVibrationOption").value ===
        "custom"
      ) {
        options.vibrate = document
          .getElementById("notificationOptionsCustomVibrate")
          .getElementsByTagName("input")[0].value;
        if ("vibrateAs" in formatOptions && formatOptions.vibrateAs === "eval")
          options.vibrate = eval(options.vibrate);
      } else
        options.vibrate =
          document.getElementById("notificationOptionsVibrationOption")
            .value === "true";
    }
    if (
      !filterDisabled ||
      !document.getElementById("notificationOptionsSoundOption").disabled
    ) {
      switch (document.getElementById("notificationOptionsSoundOption").value) {
        case "resource":
          options.sound = document
            .getElementById("notificationOptionsResourceSound")
            .getElementsByTagName("input")[0].value;
          break;
        case "online":
          options.sound = document
            .getElementById("notificationOptionsOnlineSound")
            .getElementsByTagName("input")[0].value;
          break;
        case "true":
          options.sound = true;
  ...
  function getFormatedOptionsFromForm() {
    return getOptionsFromForm({
      filter: ["disabled", "hidden"],
      vibrateAs: "eval",
      textLinesAs: "arrayOfLines"
    });
  }
  ...
  function setOptionsToForm(optionsValues, formOptions) {
    ...
    for (var method in inputsNamesByExtractionAttribute)
      for (
        var i = inputsNamesByExtractionAttribute[method].length - 1;
        i >= 0;
        i--
      )
        if (inputsNamesByExtractionAttribute[method][i] in optionsValues)
          document.getElementsByName(
            inputsNamesByExtractionAttribute[method][i]
          )[0][method] =
  ...
  function setDisabledOptionsNamesInForm(inputsNames) {
    for (var i = inputsNames.length - 1; i >= 0; i--) {
      if ("vibrate" == inputsNames[i])
        nullifyDivAndButtonFromDiv(
          document.getElementById("notificationOptionsVibrationOptionDiv")
        );
      else if ("sound" == inputsNames[i] || "soundType" == inputsNames[i])
        nullifyDivAndButtonFromDiv(
          document.getElementById("notificationOptionsSoundOptionDiv")
        );
      else
        nullifyDivAndButtonFromInput(
          document.getElementsByName(inputsNames[i])[0]
        );
    }
  }
  ...
  function showNotification() {
    if (typeof FirebaseExtendedNotification === "object")
      FirebaseExtendedNotification.showNotification(
        ...
        getFormatedOptionsFromForm()
      );
  }
  document
    .getElementById("notificationOptionsForm")
  ...
  document
    .getElementById("notificationStyle")
  ...
  function forceVibrationOption(vibrationOption, customValue) {
    document.getElementById("notificationOptionsVibrationOption").value =
      "" + vibrationOption;
    document.getElementById("notificationOptionsCustomVibrate").style.display =
      vibrationOption === "custom" ? "" : "none";
    if (typeof customValue !== "undefined")
      document
        .getElementById("notificationOptionsCustomVibrate")
        .getElementsByTagName("input")[0].value = customValue;
  }
  document
    .getElementById("notificationOptionsVibrationOption")
    .addEventListener("change", function(e) {
  ...
  function forceSoundOption(soundOption, customValue) {
    ...
    if (soundOption === "resource") {
      document.getElementById(
        "notificationOptionsResourceSound"
      ).style.display = "";
      if (typeof customValue !== "undefined")
        document
          .getElementById("notificationOptionsResourceSound")
    ...
    document.getElementById(
      "notificationOptionsSoundOption"
    ).value = soundOption;
  }
  document
    .getElementById("notificationOptionsSoundOption")
    .addEventListener("change", function(e) {
  ...
  function nullifyDiv(divElement) {
    ...
    if (!input) {
      input = divElement.getElementsByTagName("select")[0];
    }
    if (!input) {
      input = divElement.getElementsByTagName("textarea")[0];
    }
  ...
  document.getElementById("notificationOptionsForm").addEventListener(
    "click",
    function(event) {
      if (event.target.className !== "nullify") {
        return;
      }
      event.preventDefault();
  ...
  function updateGeneratedCode() {
    document.getElementById("generatedCode").innerHTML = JSON.stringify(
      {
        to:
          document.getElementById("tokenFound").innerHTML ||
  ...
  document
    .getElementById("updateGeneratedCodeButton")
    .addEventListener("click", function(e) {
      updateGeneratedCode();
    });

  document
    .getElementById("saveOptionsLocally")
    .addEventListener("click", function(e) {
      window.localStorage.setItem(
        "optionsValues",
        JSON.stringify(getOptionsFromForm())
      );
      window.localStorage.setItem(
        "disabledOptions",
        JSON.stringify(getDisabledOptionsNamesFromForm())
      );
      window.localStorage.setItem(
        "notificationStyle",
      ...
      window.localStorage.setItem(
        "soundType",
        document.getElementById("notificationOptionsSoundOption").value
      );
  ...
  (function() {
    ...
    setDisabledOptionsNamesInForm(
      JSON.parse(window.localStorage.getItem("disabledOptions"))
    );
  })();
